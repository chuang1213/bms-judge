...
## 8. 追加范围决定（2026-09-03 晚，用户）：样本空间收回三表并集

用户判断：表外谱面"牛鬼蛇神"多、质量不可控，容易污染数据。执行：

- **样本空间**：目标重新限定 sl/st/発狂2018 三表并集（`data.py` 质量围栏恢复），
  数据集回到 train 1,982 / test 1,985（cutoffs 与 3.1/3.2 时期一致，完全可比）；
- **特征侧不变**：仍然无表（26 维客观统计 + `h_knn_acc` + Phase2A 表征）——
  表只在"围栏"与"坐标"角色出现，不回到特征（PROTOCOL.md §1 已同步修订）；
- **重跑基线（C 为 3 seeds mean±std）**：

| 模型 | acc MAE | centered R² | lamp MAE | lamp QWK | BP raw MAE |
|---|---|---|---|---|---|
| A | 13.71 | — | — | — | — |
| H | **9.66** | **+0.12** | 1.94 | 0.31 | 201.1 |
| B | 10.27 | +0.08 | **1.51** | **0.61** | **185.0** |
| C0 | 13.69±1.59 | -0.44±0.20 | 1.78±0.04 | 0.50±0.02 | 235.3±6.9 |
| C1 | 14.96±2.65 | -0.75±0.51 | 1.63±0.04 | 0.56±0.01 | 211.4±9.4 |
| C2 | 15.62±1.14 | -0.79±0.12 | 1.68±0.01 | 0.54±0.01 | 230.5±14.4 |

- 结论不变式：H 主导 acc、B 主导 lamp/BP 的层级在"围栏恢复 + 特征无表"下保持；
  H 相对旧表特征版的 ~0.3 acc 代价即消融测得的 h_knn 替代成本；
- 值得注意：§8 扩样本时观察到的"Phase2A 表征稳定编码器"现象（centered R² -2.6→-0.39）
  在收回到 1,982 样本后**消失**（C1 -0.75 ≈ C2 -0.79）——进一步支持"表征的价值是
  数据规模问题，当前无法判定"的立场；
- B 的 lamp（1.51/0.61）已接近旧表特征版水平（1.469/0.627），去表代价很小。
