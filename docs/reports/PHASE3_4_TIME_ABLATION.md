...
## 11. 规模响应追踪（2026-09-04 更新）：8 玩家

新增 LED（17.8k 行，高水平）与 darklord（13k 行，首打 fail 率 59%，强挑战性玩家），
数据集扩至 8 玩家 / train 4,161 / test 4,166。C 阶梯（3 seeds）：

| 模型 | acc MAE | centered R² | lamp MAE | lamp QWK | BP raw MAE |
|---|---|---|---|---|---|
| A | 11.69 | -0.38 | 1.85 | 0.45 | 190.2 |
| H | 7.63 | +0.22 | 1.72 | 0.46 | 155.8 |
| B | **7.02** | **+0.36** | **1.41** | **0.66** | **133.9** |
| C0 | 8.96±0.14 | **+0.08±0.02** | 1.45±0.01 | 0.63±0.00 | 152.3±1.4 |
| C1 | 10.79±0.30 | -0.29±0.02 | 1.83±0.01 | 0.45±0.01 | 172.9±4.3 |
| C2 | 10.71±0.12 | -0.29±0.04 | 1.82±0.01 | 0.46±0.02 | 172.1±8.8 |

**C0 的 centered R² 首次转正**（4 玩家：≤0 → 6 玩家：-0.07 → 8 玩家：+0.08±0.02），
lamp 维持与 B 并列（1.45 vs 1.41，差距 0.04±0.01 在噪声边缘），acc 稳定（8.96±0.14，
即使加入了 fail 率 59% 的 darklord）。规模响应曲线正在兑现 readiness report §4.1 的
预判：玩家规模是 learned encoder 能否兑现的关键变量。C1/C2（事件级 chart 上下文）
在本规模仍劣于 C0，事件级 enrichment 的价值需要更大规模再判。


## 12. 追加实验（2026-09-04，用户）：Survival Scope——只预测"通过且未崩"的首打

**动机与定义**：用户认为血条死亡（FAILED）与明显崩盘（acc<50%）不属于正常完成行为，
不应作为训练目标。`data.py` 增加 `SURVIVAL_SCOPE_ONLY` 开关：目标与历史**同时**只保留
lamp∈{EASY,NORMAL,HARD}（4/5/6）且 acc≥50 的首打——模型不再知道"死亡/崩盘"这种情况存在。
原始 16,660 首打 → 保留 7,680（46.1%），每人 614–1,261；数据集 train 1,919 / test 1,924。

**结果（8 玩家，C 为 3 seeds mean±std；参照：全 scope 基线 A 11.69 / H 7.63 / B 7.02）**：

| 模型 | acc MAE | centered R² | lamp MAE | lamp QWK | BP raw MAE |
|---|---|---|---|---|---|
| A | 5.39 | -0.55 | 0.83 | 0.22 | 31.7 |
| H | 4.25 | -0.05±0.00 | 0.83 | 0.08 | 35.2 |
| B | 3.88 | +0.14 | **0.78** | 0.33 | 29.6 |
| **C0** | 4.83±0.03 | -0.34±0.02 | **0.72±0.01** | **0.34±0.01** | **27.9±0.3** |
| C1 | 5.79±0.18 | -0.85±0.09 | 0.83±0.01 | 0.21±0.03 | 31.5±1.3 |
| C2 | 5.73±0.43 | -0.80±0.23 | 0.81±0.01 | 0.23±0.01 | 35.6±3.6 |

**观察**：

1. **acc 预测大幅变准**（B 3.88 vs 全 scope 7.02）：通过局内的 acc 波动远小于"过/不过"
   的整体波动，问题本身变容易；player×chart interaction 仍存在（B cR² +0.14）但弱于全 scope；
2. **C0 首次在 lamp 与 BP 上双超 B**（0.72 vs 0.78；27.9 vs 29.6，3 seeds 一致）——
   去掉死亡/崩盘样本后，通过局之间的 lamp/BP 差异主要由玩家的近期 form 决定，
   序列编码器正是为这种"同谱面级细粒度差异"设计的；cR² 仍为负（-0.34），
   interaction 还差一步；
3. **层级结构变化**：全 scope 的排序是 H>B(acc)、B>H(lamp/BP)；survival scope 下
   B 在 acc 上仍最优，但 H 的 lamp 优势消失（H 0.83 = A 0.83，QWK 0.08 接近随机）——
   **去掉 FAILED 后 lamp 几乎不可预测**（可预测的"过/不过"信号正是被移除的部分）；
   这反证了全 scope 里 lamp 的可预测性主要来自"哪些谱会打死玩家"；
4. H 的 BP（35.2）劣于 A（31.7）：通过局内 BP 主要由谱面决定，历史统计反成噪声。

**结论与建议**：survival scope 是一个更"干净"但更窄的任务定义（预测通过局内的表现水平），
适合未来练习推荐场景（推荐谱面时默认假设玩家能通过）；但当前样本量减半且 lamp/BP 的
信息量大半来自被移除的死亡局。**建议两种 scope 并行保留**：survival 用于推荐前端的
表现水平预测，全 scope 用于交互研究；`SURVIVAL_SCOPE_ONLY` 开关已固化，切换即得。
