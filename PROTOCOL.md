...
- **样本空间（用户决定 2026-09-03；2026-09-11 扩围）**：目标限定 sl/st/発狂2018 + **normal（通常
  ☆）** 四表并集内的谱面——表外谱面质量不可控，作为质量围栏使用；表等级本身仍不作特征
  （特征契约见 chart_repr.py）。**扩围规则**：normal 在 `load_tables` 优先级**最后**
  （satellite > stella > insane > normal），已有 sl/st/発狂 标签**零改动**，只有净新增谱面进入；
  扩围后样本空间 12,725 → **13,446 行**（train 6,719 / test 6,727），**新旧空间的数字不可直接
  比较**（B 7.009→7.163、B_full 5.887→5.914，差异来自样本构成而非模型退化）；
- **难度表等级不进入任何训练特征**。
